...
Checked on `makrdevelopment.com` on 2026-09-08.

| | |
|---|---|
| Host | Ubuntu 24.04, Plesk |
| Docker | 29.8.0, Compose plugin v5.5.1 |
| Node on host | **none** — everything runs in containers |
| PostgreSQL | **16.15, native on the host**, already running |
| Subdomain | `ge.makrholdings.com`, exists, docroot holds a placeholder `index.html` |
| Custom nginx | none yet — no `vhost_nginx.conf` for this domain |
| Database | **not created yet.** Existing: `admin_daycompass`, `makr_candle`, `n8n_main` |

### Three corrections to the blind draft

**1. Do not run a Postgres container.** The host already runs PostgreSQL 16.15.
...
docker run --rm --network host -e PGPASSWORD=... postgres:16-alpine \
  psql -h 127.0.0.1 -U makr_ge -d makr_ge -At -c 'select current_database();'
# -> makr_ge
```
...
Plesk's own nginx does the serving.

## nginx on this host

Plesk owns `nginx.conf` and regenerates it; custom rules go in
`vhost_nginx.conf`, which Plesk includes inside the server block. The existing
apps do exactly this — see
`/var/www/vhosts/system/day-compass.com/conf/vhost_nginx.conf` for a working
example on this server.

For `ge.makrholdings.com`, create
`/var/www/vhosts/system/ge.makrholdings.com/conf/vhost_nginx.conf`:

```nginx
# SPA deep links. Without this, /stats and /play 404 — Plesk's nginx looks for
...
Apply with `plesk sbin nginx_control --reconfigure-domain ge.makrholdings.com`
(or Plesk's "Apache & nginx Settings" panel), then `nginx -t` before reloading.
...
Role and database `makr_ge` exist on the host's PostgreSQL 16.15, owned by
`makr_ge`, and a container using host networking connects to them successfully.

```sql
CREATE ROLE makr_ge LOGIN PASSWORD '...';
