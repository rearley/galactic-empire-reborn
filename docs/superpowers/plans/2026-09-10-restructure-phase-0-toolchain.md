...
This gives full CI on every push to the branch. The `build` job requires `github.event_name == 'push'`, so a pull request runs both suites and never builds an image. Nothing reaches ghcr and nothing reaches the watchtower on the Plesk host.
