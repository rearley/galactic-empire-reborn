...
- [x] Add `engines.node` to both `package.json` files. Pinned by
      `backend/test/unit/node-runtime-version.spec.ts` (`79d9d29`).
- [x] Backend: TypeScript 6.0.3, Jest 30, ts-jest latest (`1bd87ad`).
- [x] Frontend: React 19, Vite 8, Vitest 5, Tailwind 4 (config rewrite),
      TypeScript 6.0.3. One batch — it is only 3,521 isolated lines (`78af71d`).
...
- [x] Frontend repointed at the same declaration: `contracts.ts` and
      ...
      a SUPERSEDED note rather than deleted. (Task 4, `f9beba9`.)
...
(added `4bdd804`), but neither Dockerfile's build context was moved to the
...
**RESOLVED 2026-09-11, `cb81059`** — before phase 2 started, as this entry
...
this phase's own `d1ef6d1..HEAD` range, `VERSION` unchanged. Full before/after
...
otherwise; corrected at commit `60fff48`, filed as issue #22, full account in
...
- [x] `moduleResolution: "node"` → `"bundler"`, dropping the
      `ignoreDeprecations: "6.0"` escape hatch (`8e57f89`). Also the phase-0
      blocker that kept backend oxlint syntax-only — **oxlint now runs
      `--type-aware`** (`242e999`), its first run ever on this backend, and the
      3,441 findings were triaged into one rule disabled with a reason and six
      parked behind issues #28 and #29. No code fixed.
- [x] Jest → Vitest on the backend (`4e0f9e8`). Moved AHEAD of Nest 12 during
...
- [x] Prisma 5 → 7 on CommonJS (`2f7b2e2`): `prisma-client` generator with
      ...
      would not start.
- [x] A boot-order oracle landed BEFORE any framework move (`bcbf1a7`), and
...
unchanged in this phase's own `85f032e..HEAD` range, `VERSION` unchanged.
