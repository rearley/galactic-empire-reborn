...
1. **Fleet cap**: `noships >= MAXSHIPS` (canon default **8**, MBMGEMSG.MSG:92; sysop-tunable 1–50) → rejected (`NEW_FLEET_FULL`).
...
3. `cantexit === 0` — not in combat lock (the field is `cantexit`; there is no `tagged`).
4. In the neutral zone only the two Zygor posts (`plnum` 1 and 2) may be serviced; every other
   neutral-zone planet is refused. There is no `ZYGORCLASS` constant.
...
The six planets in sector (0,0) are created **already owned**, by the reserved id
...
| `UNIVMAX` | sysop option, `numopt(UNIVMAX,10,32767)`; canon ships **300** (a 601×601 galaxy), this port deploys **100** — 201×201 = 40,401 sectors — as a documented deviation (`docs/DECISIONS.md`) |
...
The galaxy — `(2·UNIVMAX+1)²` sectors, 40,401 at the deployed UNIVMAX of 100 — is
generated once on first boot by `GalaxyService.onModuleInit()`,
...
Sectors are visited in row-major order `y=-UNIVMAX..+UNIVMAX, x=-UNIVMAX..+UNIVMAX`. The origin sector `(0,0)`
(neutral zone) is special-cased first: it receives a fixed `s00` fixture authored as
the canonical `s00` fixture in `src/game/galaxy/s00.ts`, GENERATED by `tools/extract-s00.mjs`
from the `S00P*` blocks in `MBMGEMSG.MSG` (it is no longer hand-authored, and the .MSG is present
...
Destination coordinates are `(destX + 0.5, destY + 0.5)` where `destX` and `destY` are drawn
uniformly from `-UNIVMAX..+UNIVMAX`, i.e. anywhere in the universe square, matching the
original. Self-loop destinations (landing in the
...
Tick cadence: `interval = max(PLANTIME_MIN_SECONDS, floor(PLANTOCK_SECONDS / N))` where `N = planet count`. At 450 planets: `floor(21600/450) = 48s`. At 3 planets: `floor(21600/3) = 7200s`.
...
```
plantock = lngopt(PLANTOCK,1,32760) * 60      GEMAIN.C:469   (shipped default 360 minutes)
...
period. The economy's numbers are balanced around one `multiply()` per planet per `PLANTOCK`,
which canon ships at **360 minutes** (6 hours).
...
**Source**: GEMAIN.C:491-600 `numopt` calls, and the option blocks themselves in
`GE/REL/MBMGEMSG.MSG`.

`numopt(NAME, lo, hi)` supplies the CLAMP BOUNDS. The shipped DEFAULT is carried in the
option's own block in the .MSG — e.g. `PDAMMAX {Phaser maximum damage: 50} N 1 200`.

This section previously said the opposite: that the value "came from a sysop `.cnf` that is
not part of the reference source", and therefore that anything inside the bounds was a free
balance choice. That was wrong, and it was load-bearing — it is the reasoning that put the
whole table at clamp floors and ceilings rather than at canon. The values below are the
shipped ones, restored in the 2026-09-02 re-baseline (`docs/DECISIONS.md`).

| Constant | Canon default | numopt bounds | Notes |
|----------|---------------|---------------|-------|
| `PDAMMAX` | 50 | 1..200 | Max phaser damage. The port once ran 200 — the clamp ceiling — so every phaser type one-shot, ships dying at damage >= 100. |
| `TDAMMAX` | 35 | 1..**100** | Max torpedo damage roll. Once 200, twice the maximum the original can produce. |
| `MDAMMAX` | 25 | 1..**100** | Max missile damage roll. Once 300, three times the permitted maximum. |
| `JAMTIME` | 3 | 1..**10** | Jammer counter on deploy. Once 20, twice the maximum duration. |
| `MNDAMMAX` | 75 | 1..200 | Max mine damage. (The port's constant is named `MINEDAMMAX`; the option is `MNDAMMAX`.) |
| `PFIRDST` | 5 | 1..20 | Distance-falloff exponent in `pow(dd,factor)`. Once 1 — the clamp floor. |
| `PHATOWRP` | 5 | 0..100 | Min `phasrtype` to hit a warping victim with a normal phaser. Once 0, which let any phaser reach a ship at warp. |
| `TORFACT` | 40 raw → 4.0 | 1..50, then `/10` | Torpedo lock-quality divisor. Once raw 1. |
| `MISFACT` | 21 raw → 2.1 | 1..50, then `/10` | Missile lock-quality divisor. Once raw 1. |
| `SE100DAM` | 10 | 1..101 | Self-zap hull damage for firing inside the neutral zone. Once 101 — an instant kill. |
| `DECODDS` | 11 | 1..20 | Decoy intercept odds. |

**`DECODDS` is a different PARAMETERISATION, not a different value.** C rolls 1-in-N
(`gernd()%decodds==0`); `decoyIntercept` compares a percentage. Read the port's number as
the same option expressed the other way round.
...
floor(y)==0) calls `zaphim`: `firer.damage += SE100DAM (10)`. Because `damage >= 100` triggers
...
damage factor, every capability flag, **and `scanRange`**.

**There is no scanRange compression.** `backend/prisma/seed/ship-classes.ts` is GENERATED by
`node tools/extract-ship-classes.mjs --ts` and carries the shipped `SnnSRNG` values verbatim:
Interceptor 100 000, Stealth Fighter 200 000, Heavy Freighter 50 000, Dreadnought 500 000.
`test/balance/ship-class-canon.balance.spec.ts` checks every field against `MBMGESHP.MSG`.

This section previously described a single proportional ×0.15 compression, justified by canon
values reaching "a 50-sector radius on a 30x15 grid". Both halves were wrong. 30×15 is the scan
VIEWPORT, not the galaxy — the galaxy is `-UNIVMAX..+UNIVMAX`, 201×201 at our UNIVMAX of 100 — so
the crowding the compression existed to relieve was never real. And the compression is what caused
the scanRange drift recorded in CLAUDE.md: the seed held 15 000 while `projectRangeCell` was
written for 100 000, giving the starter ship a 1.5-sector scan through code expecting 10.
...
So an Interceptor gates weapons at 10 sectors and projects `sca lo` over 30, and a Dreadnought
gates at 50 — out-ranging everything is the flagship's defining trait, and it is canonical.

#### AI classes

Also verbatim. Note the Cybertron Battle Cruiser:

| Class | canon scanRange | gate (sectors) |
|-------|----------------:|---------------:|
| Cybertron Scout | 50 000 | 5.0 |
| Cybertron Battle Cruiser | **1 000** | 0.1 |
| Cybertron Base Star | 200 000 | 20.0 |
| Lydorian Scow | 25 000 | 2.5 |
| Murdonian Transport | 25 000 | 2.5 |
| Vakory Survey Drone | 25 000 | 2.5 |

**The Battle Cruiser's 1 000 is canon, not a typo.** `S22SRNG {Scan Range: 1000}`
(MBMGESHP.MSG:4952). This section used to call it "one correction to canon" — a dropped zero,
raised to 100 000 — on the authority of `reference/wiki/cpu-ships.md`. The wiki is a community
transcription that has been caught being wrong, and CLAUDE.md's precedence rule forbids citing it
against the shipped ship table. The seed carries 1 000.
...
The sector is the larger area. The six neutral-zone planets sit between 0.11 and 0.78 on each axis — inside
...
and most trade happens), Tahanian Station, Enforcer Planet, and the Kayriez, Lydorian and
Tryklon Portals. Found in playtest: landing on
...
port's `S00Entry` carries the same `owner` field and sets `NEUTRAL_ZONE_OWNER` on every entry,
standing in for canon's `*EMPIRE*`.

The guard here is on the SECTOR rather than on ownership: it states the rule directly and holds even
if a neutral-zone planet is somehow released. The `s00[]` table IS in the reference source — the
`S00P*` blocks of `MBMGEMSG.MSG`, extracted by `tools/extract-s00.mjs` — and it ships an owner of
`*EMPIRE*` on every entry, so nothing here is invented.
...
Current density: **40,401 sectors** (201x201 at the deployed UNIVMAX of 100) with **24 Cybertrons** seeded at boot (per-class
...
1. **C-001 scanRange gate** — the beam is capped at the firer's `scanRange`: 100 000 units
   (10 sectors) for a class 1 Interceptor.
2. **`pdamage` falloff** — damage reaches zero at `disfact = 20000 + phasrtype*4000`, i.e. 60 000
   units (6 sectors) at phasrtype 10.

A shot from across the galaxy (the square runs `-UNIVMAX..+UNIVMAX`) does nothing: `disfact`
binds long before the scanRange gate does.
...
With `TORFACT=4.0` and `MISFACT=2.1` (canon's raw 40 and 21, each divided by 10 at
GEMAIN.C:506-510), the `(5.0 - dist)` term reaches 0.7 at `dist ≈ 4.93` sectors,
...
| Heavy Freighter (3) | 200 | 0.50 | durable cargo hull — half damage |
...
5. **Per-player cap** — `MineRegistry.countByDeployer(...) >= USERMINES` — canon `USRMINES` is **3** (MBMGEMSG.MSG:372); 200 is only the option's upper clamp. Prevents carpet-bombing a sector with an entire ammo stack.
...
New constants: `HPDAMMAX=50`, `HPFIRDST=5` — canon's shipped option values. 200 and 20 are only
the numopt clamp bounds.
...
3. **Class gate** — `shieldtype === 20` ships are immune (canon's invulnerable-shield sentinel,
GEFUNCS.C:1963; no shipped class carries it); the roll returns discriminator `'skipped'` rather than `'none'` for call-site clarity.
...
- `stamp < (now - mailDays × 86400)` (default **3** days — canon's `MAILDAYS`; 7 is the option's clamp ceiling — overridable via `MIDNIGHT_MAILDAYS`)
...
2. **Team reconciliation**:
   ...
   - For each user with `teamcode > 0` and `score > 0`: `teamscore += TEAMBONU + (score / teamcount)`
     - `TEAMBONU = 500` — the sysop option (canon default 5) times 100, per GEMAIN.C:478
...
1. **AWAITING_NAME**: a connecting socket with no `Ship` row goes STRAIGHT to the name prompt —
   the gateway emits `prompt:ship-name` and the client renders `ShipNamePrompt`. There is no
   class picker: no `AWAITING_CLASS` state, no `prompt:class-list` event and no
   `ClassPickerPrompt` component exist.
...
any FULLY cloaked ship, drops AI hulls (`status === GESTAT_AUTO`), and sorts the remainder by
`shipname` case-insensitively.
...
or `@Droid-`, ordered by `score DESC, kills DESC, userid ASC`. Default cap is the canon `MAXLIST`
sysop option (default **10**); `ros all` raises the cap to 200.

**Rank and Team columns — removed.** Canon's row is
`prf("%-30s%s%5d%3d%s\r", ...)` under the ROSTER2 heading (GECMDS.C:4028, 4045): name, score,
kills, planets, empire size, and nothing else. The rank was the row's own position in an already
ordered list, and dropping the team column removed a `TeamRepository` query that ran on every
`ros`. The one deliberate deviation that remains is that the port prints `username` where C prints
`userid` — in MajorBBS the userid WAS the handle, and ours is a synthetic `usr_<hex>`.
...
- Active cloak: drains `CLOAK_ENERGY_USE` (default **7500** — canon's `CLENGUSE`, sysop-configurable via env); ramp: 1→2→10 over two ticks. `cloak===10` = fully cloaked.
...
Cost: `MAINT_COST_NORMAL=200 cr` (normal planet) / `MAINT_COST_NEUTRAL=2500 cr` (the neutral-zone posts at sector 0,0 — `plnum` 1 Zygor and 2
Tahanian Station).
...
**Balance coefficients** (all DI-injectable, env-var overrides): PLATTRT1=1.25, PLATTRT2=0.35, PLATTRF1=0.18, PLATTRF2=1.00, PLATTRF3=0.55 (canon; the old 0.05 was numopt's FLOOR), FIRETICKS=10
...
`hel` with no args returns a topic catalog (eleven groups: navigation, combat, trade, planet, ship,
maintenance, mail, comms, newprice, class, fkeys). `hel <topic>` returns the topic body. Unknown topic returns `HEL_UNKNOWN` listing valid topics. `?` is an alias bound to the same handler. Topic content is a frozen TypeScript catalog in `help-topics.ts`.
...
Retention: midnight job purges `MailStat` rows older than **3** days — canon's `MAILDAYS` default, configurable up to 7 via `MIDNIGHT_MAILDAYS`).
...
- **sector-units** (`-UNIVMAX..+UNIVMAX` on both axes — canon default 300, this port deploys 100): ship `xcoord`/`ycoord`, return value of `cdistance(a, b)`.
...
**Per-class scanRange is canon, not calibrated.** The seed is generated from `MBMGESHP.MSG` by
`tools/extract-ship-classes.mjs` and checked field-by-field by
`test/balance/ship-class-canon.balance.spec.ts`. The weapon gate is `scanRange / 10_000` sectors:

| Class | canon scanRange | gate (sectors) |
|-|-:|-:|
| Interceptor | 100 000 | 10 |
| Stealth Fighter | 200 000 | 20 |
| Heavy Freighter | 50 000 | 5 |
| Dreadnought | 500 000 | 50 |
| Cybertron Scout | 50 000 | 5 |
| Cybertron Battle Cruiser | 1 000 | 0.1 |
| Cybertron Base Star | 200 000 | 20 |
| Lydorian Scow / Murdonian / Vakory | 25 000 | 2.5 |

The table this replaces gave sub-2-sector figures for a "30×15 galaxy, diagonal ≈ 33.5 sectors".
Both halves were wrong: 30×15 is the scan viewport, and the values were the ×0.15 compression
described above, which no longer exists.
...
At the default IDAMMAX of 50 that is a scratch through shields and roughly
