...
game's first deploy to a public Plesk subdomain:
...
**Next:** the actual Plesk deploy. `docs/DEPLOYMENT.md` (new, this task) is a
...
4. **Decide on the commit author email.** Every commit carries
   `rick@makrholdings.com`, and squashing to one commit still leaves it on that
