...
interface SetOption {
  ...
  set(ship: ShipState, value: boolean, shipState: ShipStateService, users: UserRepository): Promise<void> | void;
}
 ...
 *
 * scannames/scanhome are persisted via UserRepository write-through (User.options[0/1])
...
export class SetHandlerService {
  private readonly registry: SetOption[] = [
    {
      ...
      set: async (ship, value, shipState, users) => {
  ...
  private async handle(ship: ShipState, args: string[]): Promise<CommandResult> {
    ...
    await entry.set(ship, newVal, this.shipState, this.users);
