...
import type { WarnDeps, WarnResponse } from '../../scripts/deploy-warn.mjs';

type FetchImpl = NonNullable<WarnDeps['fetchImpl']>;
type Sleep = NonNullable<WarnDeps['sleep']>;
 ...
 * deploy stalls.
 */
const okResponse = (body: unknown): WarnResponse => ({ ok: true, json: () => Promise.resolve(body) });

describe('deploy-warn pre-update hook', () => {
  it('posts the imminent phase to the local server with the ops token', async () => {
    const fetchImpl = vi.fn<FetchImpl>(() => Promise.resolve(okResponse({ notified: 1, countdownSeconds: 45 })));
  ...
  it('holds the deploy open for exactly the countdown the server asked for', async () => {
    const fetchImpl = vi.fn<FetchImpl>(() => Promise.resolve(okResponse({ notified: 2, countdownSeconds: 45 })));
    const sleep = vi.fn<Sleep>(() => Promise.resolve());
  ...
  it('does not sleep at all when nobody is in-game', async () => {
    const fetchImpl = vi.fn<FetchImpl>(() => Promise.resolve(okResponse({ notified: 0, countdownSeconds: 0 })));
    const sleep = vi.fn<Sleep>(() => Promise.resolve());
  ...
  it('does not sleep when the server rejects the call', async () => {
    const fetchImpl = vi.fn<FetchImpl>(() => Promise.resolve({ ok: false, status: 401, json: () => Promise.resolve({}) }));
    const sleep = vi.fn<Sleep>(() => Promise.resolve());
  ...
  it('says nothing, and asks for nothing, when the token is not configured', async () => {
    ...
    await warn({ fetchImpl, sleep: vi.fn<Sleep>(), env: {} });
