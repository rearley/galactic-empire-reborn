/**
 * Ship-to-ship transfers address the target by NAME. They used to pass a bare
 * `shipno`, but that is a per-user index — every captain's first hull is
 * shipno 1 — so another captain could not be named and the lookup returned
 * whichever ship sat first in the map. A bare number now means a hull in your
 * OWN fleet. @see helpers/transfer-target.ts
 */
/**
...
describe('TransferHandlerService — happy path (cargo)', () => {
  it('transfers cargo from source to target, debiting source and crediting target (SC-007)', () => {
    ...
    const result = handler.command.handler(src, ['10', 'food', 'Bob'], {}) as { lines: { text: string; category: string }[] };
  ...
  it('broadcasts TRAN_RECEIVED to the target user room', () => {
    ...
    const result = handler.command.handler(src, ['10', 'food', 'Bob'], {}) as { lines: unknown[]; broadcasts?: { room: string; event: string; payload: { text: string } }[] };
...
describe('TransferHandlerService — happy path (gold)', () => {
  it('transfers gold using "gold" keyword', () => {
    ...
    const result = handler.command.handler(src, ['100', 'gold', 'Bob'], {}) as { lines: { text: string }[] };
    expect(result.lines[0].text).toContain('Transferred');
...
describe('TransferHandlerService — rejection paths', () => {
  ...
  it('target in different sector → TRAN_SECTOR, no mutation', () => {
    ...
    const tgt = makeShip({ userid: 'u2', shipno: 2, shipname: 'Bob', xcoord: 10, ycoord: 5 }); // different x
    const { handler, mockShipState } = makeService(src, tgt);

    const result = handler.command.handler(src, ['10', 'food', 'Bob'], {}) as { lines: { text: string }[] };
  ...
  it('unknown item keyword → TRAN_UNKNOWN_ITEM, no mutation', () => {
    ...
    const result = handler.command.handler(src, ['10', 'unobtainium', 'Bob'], {}) as { lines: { text: string }[] };
