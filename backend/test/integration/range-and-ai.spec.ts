...
describe('Cybertron engagement — sees player, pursues, fires', () => {
  ...
  test('Cybertron does NOT acquire a player outside scanRange (3.0 sectors)', () => {
    const { events, fireTick, cyb } = setup(3.0);
    const acquired: unknown[] = [];
    events.on(CYBERTRON_EVENT.TARGET_ACQUIRED, (p) => acquired.push(p));
    fireTick(5);
    // cybCheckLockon uses raw cdistance (sector-units) for closest-target selection,
    // not scanRange — so it MAY still pick up an out-of-range target as "closest".
    // The real gate is whether it fires. Lock acquisition without fire is harmless.
    if (cyb.cybmine !== 255) {
      // If it locked anyway, ensure runEngagementScan didn't fire (player out of scanRange).
